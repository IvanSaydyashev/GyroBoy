...
int targetSpeed = 30;
int u_left, u_right;
float u_speed, u_balance, u_balance_old, u_lineFollower;
...
void setup() {
  ...
  regulator_b.setLimits(-20000, 20000);
  ...
  if (myTimer1.isReady()) {
    ...
    if (abs(cfAngle) < BALANCING_LIMITS) {
      ...
      */
      ...
      if (balanceSetPoint - 0.5 <= cfAngle and  cfAngle <= balanceSetPoint + 0.5) {
      ...
      Serial.print("lMotSpeed: "); Serial.print(lMotSpeed); Serial.print(" "); Serial.print("rMotSpeed: "); Serial.println(rMotSpeed);
      ...
      if (!flagSpeed) {
        if (cfAngle == 0 || (cfAngle >= 0 and cfAngle <= 1) || (cfAngle <= 0 and cfAngle >= -1)) {
          u_left = 0; u_right = 0;
        }
        flagSpeed = false;
      }
      //      u_left = constrain(u_left, -255, 255); u_right = constrain(u_right, -255, 255);
      //      Serial.print(cfAngle); Serial.print(" "); Serial.println(u_left);
